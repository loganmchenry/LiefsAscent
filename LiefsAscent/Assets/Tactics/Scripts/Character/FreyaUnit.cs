...
public class FreyaUnit : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        thisUnit = GameObject.Find("Archer");
    }
    ...
    void Update()
    {
        ...
        if (TacticalManager.takeAct == ActState.MOVE && TacticalManager.selected == thisUnit && !anim.GetCurrentAnimatorStateInfo(0).IsName("Tactics_Freya_Attack"))
        {
        ...
        if (TacticalManager.takeAct == ActState.ATTACK && TacticalManager.selected == thisUnit && !anim.GetCurrentAnimatorStateInfo(0).IsName("Tactics_Freya_Attack"))
        {
            anim.SetBool("isWalking", false);
