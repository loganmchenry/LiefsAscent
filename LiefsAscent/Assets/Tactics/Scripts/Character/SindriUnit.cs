...
public class SindriUnit : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (TacticalManager.takeAct == ActState.MOVE && TacticalManager.selected == thisUnit && !anim.GetCurrentAnimatorStateInfo(0).IsName("Tactics_Sindri_Attack"))
        {
        ...
        if (TacticalManager.takeAct == ActState.ATTACK && TacticalManager.selected == thisUnit && !anim.GetCurrentAnimatorStateInfo(0).IsName("Tactics_Sindri_Attack"))
        {
            anim.SetBool("isWalking", false);
